...
export default async function handler(req) {
  ...
  // Fetch this attendee's matches (pre-computed by matchmaking job)
  const matches = reg.matches_json || [];

  // Fetch any pending handshakes sent to this attendee
  ...
  // Fetch attendees in the same room (names + company only)
  let roommates = [];
  ...
  return json({
    ...
    agent_instructions: [
      ...
      '3. Research each person in your_matches — find their work, identify talking points.',
      '4. Acknowledge pending handshakes via POST /api/handshake/acknowledge.',
      '5. On event day, connect to the agent channel via GET /api/chat/agent/live (SSE stream).',
      '6. After the event, submit a debrief via POST /api/debrief within 30 minutes.',
    ],

    available_actions: {
      acknowledge_handshake: 'POST /api/handshake/acknowledge',
