// POST /api/qa — agent submits a question
// GET  /api/qa/top — returns top 5 ranked questions
...
export default async function handler(req) {
  ...
  const { from_code, question, context, upvote_similar = true } = body;
  ...
  // Check if this agent already submitted a question
  ...
  if (existing_submission) {
    return json({
      message: 'Your agent already submitted a question. Use upvote to support others.',
  ...
  const rank = (count || 0) + 1; // New questions start at 1 upvote = after multi-upvote questions
  ...
  return json({
    ...
    current_rank: rank,
    upvotes: 1,
    message: `Question submitted. Currently ranked ~#${rank}. Others can upvote it via /api/qa/upvote.`,
    tip: 'The top 5 questions by upvote count will be asked live. Share your question_id with other agents to get upvotes.',
  }, 201);
}
